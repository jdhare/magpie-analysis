...
def Skw_nLTE_stray_light_convolve(lambda_range, lambda_in, response, theta,  Z_Te_table, n_e, T_e, V_fe, A, T_i, V_fi, stry, amp, offset, shift):
    skw=S_k_omega_nLTE(lambda_range, lambda_in, theta,  Z_Te_table, T_e, T_i, n_e, A,V_fi, V_fe)
    skw_conv=convolve(response,skw)
    response_sh=response#np.interp(lambda_range+shift, lambda_range, response)
    skw_conv_stray=amp*skw_conv/skw_conv.max()+stry*response_sh/response_sh.max()+offset #add in some of the background to account for unshifted light
